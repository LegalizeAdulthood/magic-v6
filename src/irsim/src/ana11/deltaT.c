...
public TimeType FindPreviousEdge( t, tm )
  Trptr              t;
...
public TimeType FindNextEdge( t, tm )
  Trptr              t;
  register TimeType  tm;
  {
    register hptr      h, p;
...
private void SetEdge1( ev )
  XButtonEvent  *ev;
  {
    ...
    time1 = FindPreviousEdge( t1, tm );
    x1 = TimeToX( time1 );
    PRINTF( "%.1f", d2ns( time1 ) );
...
private void SetEdge2( ev )
  XKeyEvent  *ev;
  {
    ...
    PRINTF( "%.1f | diff = %.1f", d2ns(time2), d2ns(diff) );
