...
#define	SetDebug( FL, N )	( (debug & (FL)) and IsWatched( N ) ? 1 : 0 )
...
private void QueueFVal( nd, fval, tau, delay )
  ...
  double  tau, delay;
  {
	...
	print_final( nd, tau, delta );

    if( fval != ((ev == NULL) ? nd->npot : ev->eval) )
      {
	enqueue_event( nd, fval, delta, (Ulong) ps2d( tau ) );
      }
  }
...
private void QueueSpike( nd, spk )
  ...
  pspk  spk;
  {
    ...
    enqueue_event( nd, spk->charge, ch_delta, ch_delta );    /* spike */
    enqueue_event( nd, nd->npot, dr_delta, ch_delta );	     /* final value */
  }
	...
	for( nd = dom_pot[ dom ].nd; nd != NULL; nd = r->link.n )
	  {
	    ...
	    QueueFVal( nd, r->final, tau, delay );
	  }
...
#define NEW_THEV( T )							\
  {									\
    if( (T = thev_free) == NULL )					\
	if( (T = (Thev) MallocList( sizeof( thevenin ) )) == NULL )	\
	  {								\
	    lprintf( stderr, "RSIM: *** Out of memory (thev) ***\n" );	\
	    exit( 1 );							\
	  }								\
    thev_free = T->link.t;						\
  }									\
...
get_min_parallel( r, t )
  ...
  register tptr  t;
  {
    ...
    gmin = 1.0 / MIN( t->r.dynlow, t->r.dynhigh );
    gmax = (t->state == UNKNOWN ) ? 0.0 : gmin;

    for( t = par_list( t ); t != NULL; t = t->dcache.t )
      {
	tmp = 1.0 / MIN( t->r.dynlow, t->r.dynhigh );
	gmin += tmp;
	if( t->state != UNKNOWN )
	    gmax += tmp;
      }
...
private char *get_indent( i )
  ...
  {
    static char   indent[] = ".........................";
    static char   spaces[ sizeof( indent ) + 1 ];
    register int  c;
...
private void print_final( nd, tau, delay )
  nptr    nd;
  double  tau;
  Ulong   delay;
  {
    ...
    if( not withdriven )
	lprintf( stdout, "CS " );
    lprintf( stdout, "transition for %s: %c -> %c", pnode( nd ),
...
private void print_spike( nd, spk )
  nptr    nd;
  pspk    spk;
  {
    if( sm_stat & INCR_SIM )
	lprintf( stdout, "  [event %s->%c] ",
	 pnode( cur_node ), vchars[ cur_node->npot ] );
    else
	lprintf( stdout, "  [event #%ld: %s->%c] ", nevent,
	pnode( cur_node ), vchars[ cur_node->npot ] );
    ...
    lprintf( stdout, " (peak=%.2f delay: ch=%.1fns, dr=%.1fns)\n",
      spk->peak, ps2ns( spk->ch_delay ), ps2ns( spk->dr_delay ) );
  }
