...
public int snew_val( n, tinput )
  nptr    n;
  double  tinput;	/* not used for the switch model */
  {
	...
	if( (this->nflags & WATCHED) and debug )
	  {
	    if( sm_stat & INCR_SIM )
		lprintf( stdout, "  [event %s->%c] ",
		  pnode( cur_node ), vchars[cur_node->npot] );
	    else
		lprintf( stdout, "  [event #%ld, %s->%c] ",
		  nevent, pnode( cur_node ), vchars[cur_node->npot] );
	    if( this->nflags & INPUT )
		lprintf( stdout,"enqueuing input node %s, new value is %c\n",
		   pnode( this ), vchars[newval] );
	    else
		lprintf( stdout, "%c -> %c transition for node %s (delta = %2.1fns)\n",
		  vchars[this->npot], vchars[newval], pnode( this ),
		  d2ns( delta ) );
	  }
...
/*		if( e->ntime == cur_delta and newval == this->npot ) */
	    ...
	    if( newval != ((e == NULL) ? this->npot : e->eval) )
	      {
		if( this->nflags & WATCHED and debug )
		    lprintf( stdout, "    enqueuing final value transition\n" );
