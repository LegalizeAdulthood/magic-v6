...
private	float   min_cap_ratio = 0;		/* for safe charge sharing */
...
public tptr AddStack( t1 )
  register tptr  t1;
  {
    ...
    NEW_TRANS( ts );
    ts->gate = (nptr) t1;
    ts->ttype = t1->ttype & ~(ORED | STACKED) | GATELIST;
    ts->state = (t1->ttype & ALWAYSON) ? WEAK : UNKNOWN;
    ts->tflags = 0;
    n = (t1->source->nflags & MERGED) ? t1->drain : t1->source;
    ts->source = n;
    if( not (n->nflags & POWER_RAIL) )
	CONNECT( n->nterm, ts );

    ts->r.rstatic = ts->r.dynhigh = ts->r.dynlow = 0.0;
    cnt = 0;
    do
      {
	cnt++;
	ts->r.rstatic += t1->r.rstatic;
	ts->r.dynhigh += t1->r.dynhigh;
	ts->r.dynlow += t1->r.dynlow;

	if( (t1->state = ts->state) == UNKNOWN )	/* not ALWAYSON */
	    CONNECT( t1->gate->ngate, ts );
...
public tptr UndoStack( t1 )
  tptr  t1;
  {
    ...
    t = t1;
    n1 = n = (t->source->nflags & MERGED) ? t->drain : t->source;
    cap = 0.0;
    do
      {
	t->ttype &= ~STACKED;
