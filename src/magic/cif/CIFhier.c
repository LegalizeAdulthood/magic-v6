...
CIFInitCells()
{
    ...
    CIFTotalUse->cu_expandMask = -1;	/* This is always expanded. */
    ...
    CIFComponentUse->cu_expandMask = -1;	/* This is always expanded. */
				 ...
				 */
{
	...
	{
	    DBPaintPlane(def->cd_planes[pNum], &targetRect,
    ...
    Plane *plane;		/* Plane to check against and modify. */
{
    ...
    TiToRect(tile, &area);
    (void) DBSrPaintArea((Tile *) NULL, plane, &area,
...
#define IMPOSSIBLE 100000
    ...
    {
	TxError("CIF warning for array %s (instances of %s):\n",
		use->cu_id, use->cu_def->cd_name);
	TxError("    cells are so small and close together that two\n");
	TxError("    non-adjacent cells could have interacting CIF!\n");
    }
