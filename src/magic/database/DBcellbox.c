...
#ifndef lint
static char rcsid[] = "$Header: DBcellbox.c,v 6.0 90/08/28 18:09:29 mayo Exp $";
#endif  not lint
...
DBArrayOverlap(cu, parentRect, pxlo, pxhi, pylo, pyhi)
    register CellUse *cu;	/* Pointer to cell use which may be an array */
    ...
    void (*recurseProc)();
{
	...
	if ((parent = use->cu_parent) != (CellDef *) NULL)
	{
	    ...
	    (*recurseProc)(parent);
	    (void) GeoInclude(&use->cu_bbox, &redisplayArea);
