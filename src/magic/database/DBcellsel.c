...
#ifndef lint
static char rcsid[] = "$Header: DBcellsel.c,v 6.0 90/08/28 18:09:37 mayo Exp $";
#endif  not lint
...
dbSelectCellSr(scx, arg)
    register SearchContext *scx;	/* Context describing the cell use, the
					 ...
					 */
    register struct selectArg *arg;	/* Client data */
{
    DoubleInt childArea, bestArea, lastArea;
    ...
    /* compute childArea (using doubleints to avoid overflow). */
    {
        int xDiff, yDiff;

	xDiff = pbx->r_xtop - pbx->r_xbot;
	yDiff = pbx->r_ytop - pbx->r_ybot;
	childArea = DoubleMultII(xDiff, yDiff);
    }
	...
	*cpath->tp_next++ = '/';
    ...
    cpath->tp_next = DBPrintUseId(scx, cpath->tp_next, n);
	...
	/* compute lastArea (using doubleints to avoid overflow). */
	{
	    int xDiff, yDiff;
	  
	    xDiff = pbx->r_xtop - pbx->r_xbot;
	    yDiff = pbx->r_ytop - pbx->r_ybot;
	    lastArea = DoubleMultII(xDiff, yDiff);
	}
    }
    else
	lastArea = DIZero;
	 ...
	 */
        if (DOUBLE_EQUAL(childArea, lastArea))
	{
    ...
    if (DOUBLE_GREATER( childArea, lastArea))
    {
	if (arg->csa_bestuse != (CellUse *) NULL)
	{
	    pbx = &arg->csa_bestuse->cu_def->cd_bbox;
	    /* compute bestArea (using doubleints to avoid overflow). */
	    {
	        int xDiff, yDiff;
	  
		xDiff = pbx->r_xtop - pbx->r_xbot;
		yDiff = pbx->r_ytop - pbx->r_ybot;
		bestArea = DoubleMultII(xDiff, yDiff);
	    }
	    if (DOUBLE_GE(childArea, bestArea))
	    {
