    ...
    ClientData cdarg;	/* Argument to pass to func. */
{
	...
	(void) DBCellRead(rootUse->cu_def, (char *) NULL, TRUE);
    ...
    if (expandFlag)
    {
	(void) DBCellSrArea(&scontext, dbExpandFunc, (ClientData) &arg);
    }
    else (void) DBCellSrArea(&scontext, dbUnexpandFunc, (ClientData) &arg);
}
...
dbExpandFunc(scx, arg)
    register SearchContext *scx;	/* Pointer to search context containing
					 ...
					 */
    register struct expandArg *arg;	/* Client data from caller */
{
...
int
dbReadAreaFunc(scx)
    register SearchContext *scx;	/* Pointer to context specifying
