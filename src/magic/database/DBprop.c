...
#ifndef lint
static char rcsid[] = "$Header: DBprop.c,v 6.0 90/08/28 18:10:09 mayo Exp $";
#endif  not lint
			 ...
			 */
{
    ...
    if (cellDef->cd_props == NULL) return 0;
    htab = (HashTable *) cellDef->cd_props;

    HashStartSearch(&hs);
    while ((entry = HashNext(htab, &hs)) != NULL)
    {
	res = (*func)(entry->h_key.h_name, (ClientData) entry->h_pointer);
	if (res != 0) return res;
    }
    ...
    CellDef *cellDef;	/* Pointer to definition of cell. */
{
    ...
    if (cellDef->cd_props == NULL) return;
    htab = (HashTable *) cellDef->cd_props;
