...
#ifndef lint
static char rcsid[] = "$Header: ExtArray.c,v 6.0 90/08/28 18:15:00 mayo Exp $";
#endif  not lint
    ...
    ExtTree *et1, *et2;
{
    ...
    NODETOTILE(np, et1, tp);	/* tp = extHierLookTile(np, et1); */
    if (tp && TiGetType(tp) != TT_SPACE && extHasRegion(tp, extUnInit))
	return (extArrayTileToNode(tp, et1, ha, TRUE));

    NODETOTILE(np, et2, tp);	/* tp = extHierLookTile(np, et2); */
    if (tp && TiGetType(tp) != TT_SPACE && extHasRegion(tp, extUnInit))
	return (extArrayTileToNode(tp, et2, ha, TRUE));
...
char *
extArrayTileToNode(tp, et, ha, doHard)
    Tile *tp;
    ExtTree *et;
    HierExtractArg *ha;
			 ...
			 */
{
    ...
    register CellUse *use = ha->ha_subUse;
    register char *srcp, *dstp, *endp;
	...
	if (reg = (LabRegion *) extArrayHardNode(tp, def, ha))
	    goto found;
