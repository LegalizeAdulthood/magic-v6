...
extLengthLabels(tile, rootUse)
    register Tile *tile;	/* Some tile in extPathDef */
    CellUse *rootUse;		/* The original root cell */
{
    ...
    (void) DBTreeSrLabels(&scx, &mask, 0, &tpath, extLengthLabelsFunc,
	    (ClientData) NULL);
				 ...
				 */
{
    ...
    GeoTransPos(&scx->scx_trans, label->lab_pos, newLab->lab_pos);
...
extPathFlood(tile, p, distance, epa)
    register Tile *tile;	/* Tile whose neighbors we are to visit */
				 ...
				 */
{
	...
	for (pNum = tilePlaneNum - 1; pNum <= tilePlaneNum + 1; pNum += 2)
		...
		if (tp->ti_client == (ClientData) MINFINITY
	...
	for (pNum = PL_TECHDEPBASE; pNum < DBNumPlanes; pNum++)
		...
		(void) DBSrPaintClient((Tile *) NULL,
			...
			&DBConnectTbl[type], (ClientData) MINFINITY,
...
extPathFloodTile(srcTile, srcPoint, srcDist, dstTile, epa)
    register Tile *srcTile;	/* Tile through which we're propagating */
    Point *srcPoint;		/* Point inside or on srcTile */
    int srcDist;		/* Distance to srcPoint so far */
    register Tile *dstTile;	/* Tile on border of srcTile */
    struct extPathArg *epa;
{
    Rect srcRect, dstRect;
    Point dstPoint;
