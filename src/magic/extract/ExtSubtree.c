...
#ifndef lint
static char rcsid[] = "$Header: ExtSubtree.c,v 6.0 90/08/28 18:15:27 mayo Exp $";
#endif  not lint
    ...
    FILE *f;
{
    ...
    /*
     * Cookie-cutter up def into pieces ExtCurStyle->exts_stepSize by ExtCurStyle->exts_stepSize.
    ...
    HierExtractArg *ha;	/* Context for hierarchical extraction */
{
	...
	if (ExtOptions & EXT_DOCOUPLING)
	{
	    ...
	    HashKill(&ha->ha_cumFlat.et_coupleHash);
	}
    }
...
extSubtreeOutputCoupling(ha)
    HierExtractArg *ha;
{
    int cap, cround = ExtCurStyle->exts_capScale / 2;
	...
	NODETOTILE(ck->ck_1, &ha->ha_cumFlat, tp);
	name = extSubtreeTileToNode(tp, &ha->ha_cumFlat, ha, TRUE);
	(void) fprintf(ha->ha_outf, "cap \"%s\" ", name);

	NODETOTILE(ck->ck_2, &ha->ha_cumFlat, tp);
	name = extSubtreeTileToNode(tp, &ha->ha_cumFlat, ha, TRUE);
	(void) fprintf(ha->ha_outf, "\"%s\" %d\n", name, cap);
    }
}

/*
...
char *
extSubtreeTileToNode(tp, et, ha, doHard)
    register Tile *tp;	/* Tile whose node name is to be found */
			 ...
			 */
{
	 ...
	 * can cause problems.
	 */
	pNum = DBPlane(TiGetType(tp));
	if (pNum >= PL_PAINTBASE &&
		DBSrPaintArea((Tile *) NULL,
		    et->et_lookNames->cd_planes[DBPlane(TiGetType(tp))],
		    &r, &DBAllButSpaceBits, extConnFindFunc, (ClientData) &reg))
	{
	    if (SigInterruptPending)
		return ("(none)");
	    return (extNodeName(reg));
    ...
    if (extHasRegion(tp, extUnInit)
	    && (reg = extSubtreeHardNode(tp, et, ha)))
    {
    ...
    HierExtractArg *ha;
{
    ...
    arg.hw_mask = DBPlaneTypes[DBPlane(type)];
