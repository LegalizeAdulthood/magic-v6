...
#ifndef lint
static char rcsid[] = "$Header: ExtTimes.c,v 6.0 90/08/28 18:15:34 mayo Exp $";
#endif  not lint
...
struct cellStats
{
    ...
    int			 cs_area;	/* Total area of cell */
    int			 cs_interarea;	/* Interaction areas sum */
    int			 cs_cliparea;	/* Interaction area, counting each
    ...
    FILE *f;
{
    ...
    (void) fprintf(f, "Mean %% clipped area = %.2f\n", clip);
    (void) fprintf(f, "Mean %% interaction area = %.2f\n", inter);
    ...
    struct cellStats *cs;
{
    ...
    int extCountTiles(), extPaintOnly(), extHierCell();
    ...
    FILE *f;
{
    ...
    /* Sizes */
    (void) fprintf(f, "%8d %8d (paint) fets rects\n",
			cs->cs_fets, cs->cs_rects);
    (void) fprintf(f, "%8d %8d (hier) fets rects\n",
			cs->cs_hfets, cs->cs_hrects);
    (void) fprintf(f, "%8d %8d (flat) fets rects\n",
			cs->cs_ffets, cs->cs_frects);

    /* Times */
    (void) fprintf(f, "%8.2f %8.2f Tpaint, Tcell\n", tpaint, tcell);
    (void) fprintf(f, "%8.2f %8.2f Thier, Tincr\n", thier, tincr);

    /* Speeds */
    (void) fprintf(f, "%8.2f %8.2f (paint) fets/sec rects/sec\n",
			    fetspaint, rectspaint);
    (void) fprintf(f, "%8.2f %8.2f (hier)  fets/sec rects/sec\n",
			    fetshier, rectshier);
    (void) fprintf(f, "%8.2f %8.2f (flat)  fets/sec rects/sec\n",
			    fetsflat, rectsflat);

    /* Fraction of area in subtree processing */
    (void) fprintf(f, "%8.2f %8.2f         clip %  inter %\n",
			    pctclip, pctinter);
    ...
    FILE *f;
{
    ...
    (void) fprintf(f, "%s %8.2f %8.2f %8.2f %8.2f\n", str,
		    cum->cums_min, cum->cums_max, mean, sqrt(var));
}

/*
 * ----------------------------------------------------------------------------
