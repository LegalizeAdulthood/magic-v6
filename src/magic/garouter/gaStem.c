...
#ifndef lint
static char rcsid[] = "$Header: gaStem.c,v 6.0 90/08/28 18:52:30 mayo Exp $";
#endif  not lint
    ...
    NLNetList *netList;		/* Searched for obstructing terminals */
{
    ...
    if (ASSERT(!TTMaskEqual(&destMask, &DBZeroTypeBits), "gaStemCheckPin"))
	return ((GCRPin *) NULL);
...
gaStemNetClear(termArea, point, side, netList)
    register Rect *termArea;	/* Area of the starting terminal */
...
gaStemGridRange(type, r, pMinGrid, pMaxGrid, pStart)
    int type;
    register Rect *r;
    register int *pMinGrid, *pMaxGrid, *pStart;
{
    int min, max, start;
