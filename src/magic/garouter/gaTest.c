...
#ifndef lint
static char rcsid[] = "$Header: gaTest.c,v 6.0 90/08/28 18:52:35 mayo Exp $";
#endif  not lint
...
GAGenChans(chanType, area, f)
    ...
    FILE *f;
{
    extern int DBPaintPlane(), DBPaintPlaneVert();
    ...
    {
	case CHAN_HRIVER:
	    gaSplitPaintPlane = DBPaintPlane;
    ...
    FILE *f;
{
    ...
    (void) fprintf(f, "garoute channel %d %d %d %d",
		r.r_xbot, r.r_ybot, r.r_xtop, r.r_ytop);
    if (TiGetType(tile) != TT_SPACE)
	(void) fprintf(f, " %s", gaSplitType == CHAN_HRIVER ? "h" : "v");
    (void) fprintf(f, "\n");
    return (0);
}

