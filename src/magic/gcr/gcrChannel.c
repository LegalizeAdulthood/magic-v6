...
#ifndef lint
static char rcsid[] = "$Header: gcrChannel.c,v 6.0 90/08/28 18:37:44 mayo Exp $";
#endif  not lint
    ...
    int width;		/* Width of the channel (# rows) */
{
    ...
    for (i = 0; i < lenWds; i++)
    {
	MALLOC(short *, ch->gcr_result[i], nBytes);
    ...
    GCRChannel *ch;		/* Pointer to channel structure to be freed. */
{
    ...
    FREE((char *) ch->gcr_tPins);
    FREE((char *) ch->gcr_bPins);
    FREE((char *) ch->gcr_lPins);
    FREE((char *) ch->gcr_rPins);
    for (net = ch->gcr_nets; net; net = net->gcr_next)
	FREE((char *) net);
    FREE((char *) ch->gcr_lCol);

    FREE((char *) ch->gcr_dRowsByCol);
    FREE((char *) ch->gcr_dColsByRow);

#ifdef	IDENSITY
	/* For debugging */
    FREE((char *) ch->gcr_iRowsByCol);
    FREE((char *) ch->gcr_iColsByRow);
#endif	IDENSITY

    FREE((char *) ch->gcr_density);
    for (i = 0; i <= ch->gcr_length + 1; i++)
	FREE((char *) ch->gcr_result[i]);
    FREE((char *) ch->gcr_result);
    FREE((char *) ch);
}

/*
...
GCRFlipLeftRight(src, dst)
    register GCRChannel *src;	/* Original channel. */
    register GCRChannel *dst;	/* Channel to be modified to contain
