...
#ifndef lint
static char rcsid[] = "$Header: gcrDebug.c,v 6.0 90/08/28 18:37:48 mayo Exp $";
#endif  not lint
    ...
    char *fname;
{
    ...
    MALLOC(GCRChannel *, ch, sizeof (GCRChannel));
    ch->gcr_type = CHAN_NORMAL;
    ch->gcr_area = box;
    ch->gcr_transform = trans;
    ...
    (void) fclose(fp);
    MALLOC(GCRColEl *, ch->gcr_lCol, (ch->gcr_width+2) * sizeof (GCRColEl));
    times(&tbuf1);
    (void) GCRroute(ch);
    times(&tbuf2);
    ...
    FILE *fp;
{
    ...
    MALLOC(int *, ch->gcr_density, lenWds * sizeof (int));
    ch->gcr_lPins = gcrMakePinLR(fp, 0, ch->gcr_width);
    MALLOC(GCRPin *, ch->gcr_tPins, lenWds * sizeof (GCRPin));
    MALLOC(GCRPin *, ch->gcr_bPins, lenWds * sizeof (GCRPin));
    MALLOC(short **, ch->gcr_result, lenWds * sizeof (short *));

    /* Initialize end columns */
    MALLOC(short *, ch->gcr_result[0], widWds * sizeof (short));
    MALLOC(short *, ch->gcr_result[ch->gcr_length+1], widWds * sizeof (short));
    for (i = 0; i < widWds; i++)
    {
	ch->gcr_result[0][i] = 0;
    ...
    {
	/* Allocate the column */
	MALLOC(short *, ch->gcr_result[i], widWds * sizeof (short));
	...
	ch->gcr_bPins[i].gcr_pId = (GCRNet *) pid;
	...
	ch->gcr_tPins[i].gcr_pId = (GCRNet *) pid;
...
gcrMakePinLR(fp, x, size)
    ...
    int x, size;
{
    register GCRPin *result;
    register int i;

    MALLOC(GCRPin *, result, (unsigned) (size+2) * sizeof (GCRPin));
...
#define	NETID(pin)	((pin).gcr_pId ? (pin).gcr_pId->gcr_Id : 0)
    ...
    for (j = 1; j <= ch->gcr_width; j++)
	(void) fprintf(fp, "%d ", NETID(ch->gcr_lPins[j]));
    (void) fprintf(fp, "\n");
    ...
    {
	/* Bottom pin */
	(void) fprintf(fp, "%d ", NETID(ch->gcr_bPins[i]));
	...
	for (j = 1; j <= ch->gcr_width; j++)
	{
	    ...
	    {
		case 0:			(void) fprintf(fp, ". "); break;
		case GCRBLKM:		(void) fprintf(fp, "m "); break;
		case GCRBLKP:		(void) fprintf(fp, "p "); break;
		case GCRBLKM|GCRBLKP:	(void) fprintf(fp, "x "); break;
	    }
	}
    ...
    for (j = 1; j <= ch->gcr_width; j++)
	(void) fprintf(fp, "%d ", NETID(ch->gcr_rPins[j]));
    (void) fprintf(fp, "\n");
    (void) fclose(fp);
}

    ...
    int chanDensity;
{
    ...
    (void) fprintf(fp, "Chan width: %d\n", ch->gcr_width);
    (void) fprintf(fp, "Chan length: %d\n", ch->gcr_length);
    (void) fprintf(fp, "Chan area: ll=(%d,%d) ur=(%d,%d)\n",
		ch->gcr_area.r_xbot, ch->gcr_area.r_ybot,
		ch->gcr_area.r_xtop, ch->gcr_area.r_ytop);
    (void) fprintf(fp, "Max column density (global):  %d\n", ch->gcr_dMaxByCol);
    (void) fprintf(fp, "Max column density (channel): %d\n", chanDensity);
    (void) fprintf(fp, "Column density by column:\n");
    (void) fprintf(fp, "%3s %5s", "COL", "GLOB");
#ifdef	IDENSITY
    (void) fprintf(fp, " %5s %5s", "INIT", "DIFF");
#endif	IDENSITY
    (void) fprintf(fp, " %5s\n", "CHAN");
    for (i = 1; i <= ch->gcr_length; i++)
    {
	(void) fprintf(fp, "%3d %5d", i, ch->gcr_dRowsByCol[i]);
	diff = ch->gcr_dRowsByCol[i];
#ifdef	IDENSITY
	diff -= ch->gcr_iRowsByCol[i];
	(void) fprintf(fp, " %5d %5d", ch->gcr_iRowsByCol[i], diff);
#endif	IDENSITY
	(void) fprintf(fp, "%5d%s\n", ch->gcr_density[i],
		(diff != ch->gcr_density[i]) ? " *****" : "");
    }
    (void) fprintf(fp, "------\n");
    (void) fprintf(fp, "Row density by column (global only):\n");
    (void) fprintf(fp, "%3s %5s", "ROW", "GLOB");
#ifdef	IDENSITY
    (void) fprintf(fp, " %5s %5s", "INIT", "DIFF");
#endif	IDENSITY
    (void) fprintf(fp, "\n");
    ...
    {
	(void) fprintf(fp, "%3d %5d", i, ch->gcr_dColsByRow[i]);
#ifdef	IDENSITY
	(void) fprintf(fp, " %5d %5d", ch->gcr_iColsByRow[i],
