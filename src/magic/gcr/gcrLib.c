...
#ifndef lint
static char rcsid[] = "$Header: gcrLib.c,v 6.0 90/08/28 18:38:02 mayo Exp $";
#endif  not lint
...
gcrBlocked(col, i, net, last)
    ...
    int last;
{
    register GCRColEl *colptr = &col[i];
...
gcrUnlinkTrack(col, toUnlink)
    ...
    int toUnlink;
{
    register GCRColEl *colPtr = &col[toUnlink];
    ...
    int size;
{
    ...
    MALLOC(GCRColEl *, result, (size+2) * sizeof (GCRColEl));
    limit = size + 2;
    for (i = 0; i < limit; i++)
	result[i] = col[i];
    return (result);
