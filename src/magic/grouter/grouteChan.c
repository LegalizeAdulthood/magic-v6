    ...
    GCRChannel *ch;
{
	...
	{
	    TITORECT(tile, &r);
	    (void) sprintf(mesg, "Tile client %x doesn't match chan %x",
		    tile->ti_client, ch);
	    DBWFeedbackAdd(&r, mesg, EditCellUse->cu_def,
		    1, STYLE_MEDIUMHIGHLIGHTS);
	}
    ...
    GCRChannel *ch;
{
	...
	(void) sprintf(mesg, "After clipping chan %x", ch);
    ...
    Tile *tile;
{
    ...
    (void) sprintf(mesg, "tile ch=%x type=%d",
		tile->ti_client, TiGetType(tile));
    TxMore(mesg);
    ...
    register Rect *area;
{
    ...
    register TileType type = TiGetType(tile);
...
glChanMergeFunc(tile)
    register Tile *tile;
{
    register GCRChannel *ch = (GCRChannel *) tile->ti_client;
    register Tile *tp;
    int ret;
    ...
    register GCRChannel *ch;
{
	...
	for (lo = 1; lo < dCol->dm_size; lo++)
	{
		...
		MALLOC(PaintArea *, pa, sizeof (PaintArea));
		pa->pa_area = area;
    ...
    GCRPin *pin;	/* Find tile containing this pin */
{
    ...
    ASSERT(tp->ti_client != (ClientData) MINFINITY, "glChanPinToTile");
