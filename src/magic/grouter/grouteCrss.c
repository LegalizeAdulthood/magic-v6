...
#ifndef lint
static char rcsid[] = "$Header: grouteCrss.c,v 6.0 90/08/28 18:46:53 mayo Exp $";
#endif  not lint
...
glCrossUnreserve(net)
    NLNet *net;
{
    register NLTermLoc *loc;
    register NLTerm *term;
    register GCRPin *pin;
    ...
    ClientData cdata;		/* Passed to (*func)() */
{
    ...
    register GCRPin *pins, *pin;

    /* Sanity checks: callers should ensure that these are true */
    ASSERT(tp->ti_client != (ClientData) MINFINITY, "glCrossEnum");
...
glCrossChoose(newRest, tp, pin, newPath)
    register GlPoint *newRest;	/* Portion of path already assigned */
    register Tile *tp;		/* UNUSED */
    register GCRPin *pin;	/* Pin on boundary of tp being considered */
...
glCrossCost(lookAhead, exitPt, entryPt)
    ...
    GlPoint *entryPt;
{
    register GCRPin *entryPin, *exitPin, *otherPin;
    GCRPin *opposite;
    int count, cost;
