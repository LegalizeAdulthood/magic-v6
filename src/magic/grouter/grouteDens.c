...
#ifndef	lint
static char rcsid[] = "$Header: grouteDens.c,v 6.1 90/08/28 19:15:46 mayo Exp $";
#endif	lint
    ...
    int top, cap;
{
    ...
    dm->dm_size = top + 1;
    dm->dm_cap = cap;
    CALLOC(short *, dm->dm_value, sizeof (short) * dm->dm_size);
}

/*
 * ----------------------------------------------------------------------------
...
glDMMaxInRange(dm, lo, hi)
    ...
    int lo, hi;
{
    register short *dval = dm->dm_value;
    register int n, max;
...
Void
glDensInit(dmap, ch)
    register DensMap dmap[2];
    GCRChannel *ch;
{
    register short *dSrc, *dDst, *dEnd;
