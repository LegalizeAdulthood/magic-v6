...
#ifndef lint
static char rcsid[] = "$Header: grouteMaze.c,v 6.0 90/08/28 18:47:03 mayo Exp $";
#endif  not lint
    ...
    NLTermLoc *loc;	/* Destination point */
{
    ...
    HeapAdd(&glMazeHeap, &cost, (char *) outPt);
    glCrossingsAdded++;
}

/*
    ...
    register GlPoint *inPt;
{
    ...
    register GCRChannel *inCh = inPin->gcr_ch;
    register int cost;
    Tile *outTile;
    GlPoint *outPt;
    ...
    {
	int costInMem = cost;
	HeapAdd(&glMazeHeap, &costInMem, (char *) outPt);
    }
    glCrossingsAdded++;
...
glMazePropNormal(inPt)
    GlPoint *inPt;
{
    register Tile *tile = inPt->gl_tile, *tp;
...
glMazeTile(inPt, tile, dir)
    ...
    register Tile *tile;	/* Tile adjacent to inPt->gl_tile */
...
glMazeTileFunc(inPt, tp, pin)
    register GlPoint *inPt;	/* Top of heap point being expanded */
    register Tile *tp;		/* Tile adjacent to inPt->gl_tile */
    register GCRPin *pin;	/* Available pin on boundary of tp */
{
    GlPoint *outPt;
    int cost;
