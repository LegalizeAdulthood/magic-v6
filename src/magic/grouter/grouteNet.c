				 ...
				 */
{
    ...
    glNumTries++;
    if (glLogFile)
	(void) fprintf(glLogFile, "---\t%d\t0,0\t0\t0\n", glNumTries);
	...
	if (inCh != loc->nloc_chan || !glFinalPropagate(inPt, loc))
	{
		...
		glNormalPropagate(inPt, inCh, hEntry.he_key);
	}
    ...
    HeapEntry *hEntry;	/* See above */
{
	    ...
	    && ((GlPoint *) bestCostTop->he_id)->gl_next == PROCESSED_MARK)
	{
	    ...
	    {
		TxPrintf("Discarding point (cost=%d): ", bestCostTop->he_key);
	...
	{
	    if (bestCostTop == NULL || glHeapTop->he_key < bestCostTop->he_key)
	    {
		if (DebugIsSet(glDebugID, glDebHeap))
	...
	newBestCost = bestCostTop->he_key;
	...
	while ((glHeapTop = HeapRemoveTop(&glHeap, hEntry))
		&& glHeapTop->he_key <= minAcceptableCost)
	{
		...
		TxPrintf("Move to prox (pcost=%d,cost=%d): ",
			dist, glHeapTop->he_key);
		glPrintPoint((GlPoint *) glHeapTop->he_id);
		TxPrintf("\n");
	    }
	    HeapAdd(&glBestHeap, glHeapTop->he_key, glHeapTop->he_id);
	    HeapAdd(&glProximityHeap, dist, glHeapTop->he_id);
	}

	if (glHeapTop)
	    HeapAdd(&glHeap, glHeapTop->he_key, glHeapTop->he_id);
    }
    ...
    *pNewBest = (topPt == glBestPt);
	...
	TxPrintf("Returning point (%s, cost=%d): ",
			*pNewBest ? "best" : "not best", hEntry->he_key);
	glPrintPoint(topPt);
	TxPrintf("\n");
    }
    return TRUE;
    ...
    NLTermLoc *loc;	/* Destination point */
{
    ...
    HeapAdd(&glHeap, cost, (char *) outPt);
    if (glLogFile)
    {
	(void) fprintf(glLogFile, "FIN\t%d\t%d,%d\t%d\n",
		glNumTries,
...
glRiverPropagate(inPt)
    register GlPoint *inPt;
{
    register GCRPin *inPin = inPt->gl_pin, *outPin, *linkedPin;
    register GCRChannel *inCh = inPt->gl_ch;
    register int cost;
    GlPoint *outPt;
	...
	if (cost < linkedPin->gcr_cost)
	{
	    ...
	    if (glLogFile)
	    {
		(void) fprintf(glLogFile, "RIV\t%d\t%d,%d\t%d\t%d\n",
			glNumTries,
			outPt->gl_point.p_x, outPt->gl_point.p_y,
...
glNormalPropagate(inPt, inCh, heapCost)
    register GlPoint *inPt;	/* Point on the boundary of inCh */
    register GCRChannel *inCh;	/* Channel through which we're passing */
...
#define	YCOST(y, pin)	ABSDIFF((y), (pin)->gcr_point.p_y)
	...
	HeapAdd(&glHeap, cost-1, (char *) inPt);
	glCrossingsPartial++;
    }
...
glMinRemainingCost(inPt, inCh, pRect, dRect)
    GlPoint *inPt;
    register GCRChannel *inCh;
    register Rect *pRect, *dRect;
{
    register int cost, n;
    register GCRPin *pins;
...
glPropagateFn(outCh, outPin, inPt)
    ...
    register GCRPin *outPin;	/* Exit pin, also in inCh */
    register GlPoint *inPt;	/* Point being considered */
{
    register int cost, n;
    int finalCost;
    GlPoint *outPt;
    ...
    HeapAdd(&glHeap, finalCost, (char *) outPt);
    if (glLogFile)
    {
	(void) fprintf(glLogFile, "ADD\t%d\t%d,%d\t%d\t%d\n",
		glNumTries,
		outPt->gl_point.p_x, outPt->gl_point.p_y,
...
glCrossPenalty(cost, inCh, outCh, inPin, outPin)
    ...
    GCRChannel *outCh;		/* Channel to which outPin exits, or NULL
				 ...
				 */
    register GCRPin *inPin;	/* Pin used to enter inCh */
    register GCRPin *outPin;	/* Pin used to exit inCh into outCh */
{
    register GCRPin *otherPin;
    int count;
    ...
    int cost;
{
    ...
    {
	(void) fprintf(glLogFile, "TOP\t%d\t%d,%d\t%d\t%d\n",
		glNumTries,
