...
#ifndef	lint
static char rcsid[] = "$Header: groutePen.c,v 6.1 90/08/28 19:15:59 mayo Exp $";
#endif	lint
...
glPenSetPerChan(net)
    NLNet *net;
{
    register CZone *czNet, *czChan;
    register GlobChan *gc;
	...
	MALLOC(CZone *, czChan, sizeof (CZone));
	*czChan = *czNet;
...
glPenScanDens(czList, ch, dm, type)
    ...
    register DensMap *dm;	/* Map to search */
    int type;			/* Type of zone: CZ_ROW or CZ_COL */
{
    register short *val = dm->dm_value;
    register CZone *cz;
    register int i;
			 ...
			 */
{
     ...
     * NetSet.
     */
    MALLOC(NetSet **, crossArray, numCross * sizeof (NetSet *));
	...
	ns = *nsap++;
	nc = (NetClient *) ns->ns_net->nnet_cdata;
	MALLOC(CZone *, czNew, sizeof (CZone));
	*czNew = *cz;
...
glPenFindCrossingNets(cz, netList)
    register CZone *cz;	/* A single CZone being processed */
    NLNetList *netList;	/* List of all nets; we look for nets that cross
			 * this zone.
			 */
{
    ...
    struct glCrossClient *rcc;
{
    ...
    MALLOC(NetSet *, ns, sizeof (NetSet));
...
glPenEnumCross(cz, rp, func, cdata)
    register CZone *cz;		/* Look for segments passing through here */
    register GlPoint *rp;	/* List of GlPoints (linked by gl_path ptrs) */
    int (*func)();		/* Apply to each segment passing through cz */
    ClientData cdata;		/* Passed to (*func)() */
{
    register GCRPin *srcPin, *dstPin;
    int cSrc, cDst;
...
glPenDeleteFunc(cz, srcPin, dstPin, dm)
    ...
    register DensMap *dm;	/* Remove srcPin..dstPin segment from 'dm' */
{
    register int n;
    int lo, hi;
