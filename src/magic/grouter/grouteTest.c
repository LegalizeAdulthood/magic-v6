...
#ifndef lint
static char rcsid[] = "$Header: grouteTest.c,v 6.1 90/08/28 19:16:03 mayo Exp $";
#endif  not lint
    ...
    int heapPtsBefore, frontierPtsBefore, startPtsBefore;
{
    ...
    MALLOC(GlNetHisto *, gh, sizeof (GlNetHisto));
...
glHistoDump()
{
    ...
    if (count > 0)
	(void) fprintf(fp, "%d: %d\n", lastsize, count);
    (void) fprintf(fp, "TOTAL: %d\n", total);

    /* Output sorted by number of frontier points */
    (void) fprintf(fp, "--------- by frontier points ---------\n");
    HeapInit(&histoHeap, 128, FALSE, FALSE);
    for (gh = glNetHistoList; gh; gh = gh->glh_next)
	HeapAdd(&histoHeap, &(gh->glh_frontier), (char *) gh);
    count = lastsize = total = 0;
    while (HeapRemoveTop(&histoHeap, &entry))
    {
	gh = (GlNetHisto *) entry.he_id;
