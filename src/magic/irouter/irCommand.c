...
#ifndef lint
static char rcsid[] = "$Header: irCommand.c,v 6.1 90/08/28 19:23:44 mayo Exp $";
#endif  not lint
...
/* irWzdSetExpandDests-- */
Void
irWzdSetExpandDests(s, file)
char *s;
FILE *file;
{
    SetNoisyBool(&(irMazeParms->mp_expandDests),s,file);
}
...
static struct
{
    ...
    int    (*cP_proc)();	/* Procedure processing this parameter */
} cParms[] = {
    "active",	irCSetActive,
    "width",	irCSetWidth,
    "cost",	irCSetCost,
...
( \
  (cmd->tx_argc <= 4) ? NULL : \
    (nV_i >= cmd->tx_argc-1) ? cmd->tx_argv[nV_i=4] : cmd->tx_argv[++nV_i] \
)
    ...
    TxCommand *cmd;
{
    ...
    int which, n;
    int nV_i = cmd->tx_argc-1; 	/* Used by NEXTVALUE, must be initialized */
    
    /* Process by case */
    if(cmd->tx_argc == 2 ||
	(cmd->tx_argc == 3 && (strcmp(cmd->tx_argv[2],"*")==0)) ||
	(cmd->tx_argc >= 4 && 
	...
	(cmd->tx_argc >= 4 && (strcmp(cmd->tx_argv[3],"*")==0)))
    {
    ...
    else if(cmd->tx_argc>=4 && strcmp(cmd->tx_argv[2],"*")==0)
    {
...
static struct
{
    ...
    int    (*lP_proc)();	/* procedure processing this parameter */
} lParms[] = {
    "active",	irLSetActive,
    "width",	irLSetWidth,
    "hCost",	irLSetHCost,
    ...
    TxCommand *cmd;
{
	...
	switch (which) 
	{
		...
		if(!StrIsInt(argv[i]))
		{
		    TxError("Noninteger coordinate: %s\n",argv[i]);
		    goto leaveClean;
		}
		destRect.r_xbot = atoi(argv[i]);
    ...
    irRoute(w,startType,startPtArg,startLabel,startLayers,
...
static struct
{
    ...
    int    (*srP_proc)();	/* Procedure processing this parameter */
} srParms[] = {
    "rate",		irSrSetRate,
    "width",		irSrSetWidth,
    0
...
static struct
{
    ...
    int    (*wzdP_proc)();	/* Procedure processing this parameter */
} wzdParms[] = {
    "bloom",		irWzdSetBloom,
    "boundsIncrement",	irWzdSetBoundsIncrement,
    "estimate",		irWzdSetEstimate,
    "expandDests",	irWzdSetExpandDests,
    ...
    TxCommand *cmd;
{
	...
	(void) fprintf(saveFile,":iroute contact %s * ",
		       DBTypeLongNameTbl[rC->rc_routeType.rt_tileType]);
	for(n=0; cParms[n].cP_name; n++)
	{
	...
	(void) fprintf(saveFile,":iroute layer %s * ", 
	    DBTypeLongNameTbl[rL->rl_routeType.rt_tileType]);
	for(n=0; lParms[n].lP_name; n++)
	{
	...
	for(n=0; srParms[n].srP_name; n++)
	{
	    (void) fprintf(saveFile,":iroute search %s ", 
			   srParms[n].srP_name); 
	...
	(void) fprintf(saveFile,":iroute spacings CLEAR\n");
	for (rT=irRouteTypes; rT!= NULL; rT=rT->rt_next)
	{
	    for (i=0;i<TT_MAXTYPES;i++)
		if(rT->rt_spacing[i]>=0)
		    (void) fprintf(saveFile,":iroute spacings %s %s %d\n", 
				   DBTypeLongNameTbl[rT->rt_tileType],
				   DBTypeLongNameTbl[i],
				   rT->rt_spacing[i]);
	    if(rT->rt_spacing[TT_SUBCELL]>=0)
	...
	for(n=0; wzdParms[n].wzdP_name; n++)
	{
	    (void) fprintf(saveFile,":iroute wizard %s ", 
