...
#ifndef lint
static char rcsid[] = "$Header: irRoute.c,v 6.1 90/08/28 19:24:00 mayo Exp $";
#endif  not lint
...
irRoute(cmdWindow, startType, argStartPt, argStartLabel, argStartLayers, 
    ...
    Window *cmdWindow;	/* window route command issued to */
    ...
    List *argDestLayers;	/* OK route layers to end route on */
{
    ...
    /* Do the Route */
    path = MZRoute(); /* If MZRoute is interrupted it returns best path
		       * found so far.
		       */
 ...
 */
Point
irGetStartPoint(startType, argStartPt, argStartLabel, routeUse)
    int startType;		/* how start is specified */
    ...
    CellUse *routeUse;		/* toplevel cell visible to router */
{
	...
	{   
	    ...
	    {
		TxError("Start label not unique:  ");
		TxError("more than one '%s' label selected!\n", 
			argStartLabel);
		goto abort;
	    }
		...
		if (lSD.lsd_result == LSR_NOTUNIQUE)
		{
		    TxError("Start label '%s' not unique:  select one.\n",
			    argStartLabel);
		    goto abort;
		}
		else if (lSD.lsd_result == LSR_NOTFOUND)
 ...
 */
Rect
irGetDestRect(destType, argDestRect, argDestLabel, routeUse)
    int destType;		/* how dest is specified */
    ...
    CellUse *routeUse;		/* toplevel cell visible to router */
{
	...
	{   
	    ...
	    if (lSD.lsd_result == LSR_NOTUNIQUE)
	    {
		TxError("Destination label not unique:  ");
		TxError("more than one '%s' label selected!\n", argDestLabel);
		goto abort;
	    }
	    else if (lSD.lsd_result == LSR_NOTFOUND)
		...
		if (lSD.lsd_result == LSR_NOTUNIQUE)
		{
		    TxError("Destination label '%s' not unique:  ",
			    argDestLabel);
		    TxError("select one.\n");
		    goto abort;
		}
 ...
 */
    /*ARGSUSED*/
int
irSelLabelsFunc(label, cellDef, transform, lsd)
Label *label;
CellDef *cellDef;
Transform *transform;
ClientData lsd;
{
    if (strcmp(((LabelSearchData *)lsd)->lsd_name, label->lab_text)!=0)
    {
	/* this label doesn't match, continue search */
	return 0;
    }
    else if (((LabelSearchData *)lsd)->lsd_result == LSR_FOUND)
    {
	/* second match, set result and terminate search */
	((LabelSearchData *)lsd)->lsd_result = LSR_NOTUNIQUE;
	return 1;
    }
    else
    {
	...
	GeoTransRect(transform,
		&(label->lab_rect),
		&(((LabelSearchData *)lsd)->lsd_locRect));
	((LabelSearchData *)lsd)->lsd_result = LSR_FOUND;
	return 0;
    }
}
 ...
 */
    /*ARGSUSED*/
int
irAllLabelsFunc(rect, name, label, lsd)
Rect *rect;
char *name;
Label *label;
ClientData lsd;
{
    if (((LabelSearchData *)lsd)->lsd_result == LSR_FOUND)
    {
	...
	((LabelSearchData *)lsd)->lsd_locRect = *rect;
	((LabelSearchData *)lsd)->lsd_result = LSR_FOUND;
	return 0;
    }
}
