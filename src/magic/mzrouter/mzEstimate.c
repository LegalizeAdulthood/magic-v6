...
#ifndef lint
static char rcsid[] = "$Header: mzEstimate.c,v 6.1 90/08/28 19:35:04 mayo Exp $";
#endif  not lint
...
typedef struct vertex
{
    ...
    DoubleInt vx_cost;	/* Min cost from here to destination point */
} Vertex;
 ...
 * mzCleanEstimate --
 *
 * Clear estimate plane and reclaim storgage.
 *
 * Results:
 *	None
 *
    ...
    TileCosts *spaceCosts; /* costs to assign to space tiles */
{
    ...
    MALLOC(TileCosts *, newCosts, sizeof(TileCosts));
    ...
    register Tile *tile;
{
	...
	if(RIGHT(tile)<=MAX_FINITE_COORDINATE)
	{
	    ...
	    {
		/* Upper Right */
		tRT = tile->ti_rt;	/* right top corner stitch (up) */
		tTR = tile->ti_tr;	/* top right corner stitch (to right) */
	...
	if (vLLeft)
	{
	    MALLOC(Estimate *, e, sizeof(Estimate));
	    e->e_x0 = LEFT(tile);
	    e->e_y0 = BOTTOM(tile);
    ...
    register Tile *tile;
{
	...
	if(TiGetType(tSolid) == TT_EST_DEST)
	{
	    ...
	    e->e_x0 = RIGHT(tile);
	    e->e_y0 = 0;
	    e->e_cost0 = DoubleMultII((LEFT(tSolid) - RIGHT(tile)), 
				      tc->tc_hCost);
	    e->e_hCost = tc->tc_hCost;
	    e->e_vCost = 0;
	    e->e_next = tc->tc_estimates;
	...
	if(TiGetType(tSolid) == TT_EST_DEST)
	{
	    ...
	    e->e_x0 = LEFT(tile);
	    e->e_y0 = 0;
	    e->e_cost0 = DoubleMultII((RIGHT(tSolid) - LEFT(tile)), 
				      tc->tc_hCost);
	    e->e_hCost = tc->tc_hCost;
	    e->e_vCost = 0;
	    e->e_next = tc->tc_estimates;
	...
	if(TiGetType(tSolid) == TT_EST_DEST)
	{
	    ...
	    e->e_x0 = 0;
	    e->e_y0 = TOP(tile);
	    e->e_cost0 = DoubleMultII((BOTTOM(tSolid) - TOP(tile)), 
				      tc->tc_vCost);
	    e->e_hCost = 0;
	    e->e_vCost = tc->tc_vCost;
	    e->e_next = tc->tc_estimates;
	...
	if(TiGetType(tSolid) == TT_EST_DEST)
	{
	    ...
	    e->e_x0 = 0;
	    e->e_y0 = BOTTOM(tile);
	    e->e_cost0 = DoubleMultII((TOP(tSolid) - BOTTOM(tile)), 
				      tc->tc_vCost);
	    e->e_hCost = 0;
	    e->e_vCost = tc->tc_vCost;
	    e->e_next = tc->tc_estimates;
 ...
 * ----------------------------------------------------------------------------
 */
bool AlwaysAsGood(est1, est2, tile) 
Estimate *est1;
Estimate *est2;
Tile *tile;
{
    if(DOUBLE_GREATER(est1->e_cost0,est2->e_cost0))
    {
	return FALSE;
    }
    else
	...
	{
	    DoubleInt hCost, vCost, cost;

	    hCost = DoubleMultII(est1->e_hCost, 
				 ABS(est2->e_x0 - est1->e_x0));
	    vCost = DoubleMultII(est1->e_vCost, 
				 ABS(est2->e_y0 - est1->e_y0));
	    
	    DOUBLE_ADD(cost, hCost, vCost);
	    DOUBLE_ADD(cost, cost, est1->e_cost0);

	    return (DOUBLE_LE(cost, est2->e_cost0));
	}
    }
}
...
mzSplitTiles(plane, point)
    Plane * plane;
    register Point * point;     /* origin from which tiles split */
{
...
mzAssignVertexCosts()
{
    ...
    /* Initialize Heap */
    HeapInitType(&adjHeap, 1024, FALSE, FALSE, HE_DINT);
    ...
    Heap *adjHeap;
{
	...
	/* adjust cost */
	{
	    ...
	    newCost = DoubleMultII(rate, distance);
	    DOUBLE_ADD(newCost, newCost, vxThis->vx_cost);

	    if(DOUBLE_LESS(newCost, vxAbove->vx_cost))
	    {
		vxAbove->vx_cost = newCost;
		HeapAdd(adjHeap, &newCost, (char *) vxAbove);
	    }
	}
noAbove:;
    }
	...
	/* adjust cost */
	{
	    ...
	    if(DOUBLE_LESS(newCost, vxRight->vx_cost))
	    {
		vxRight->vx_cost = newCost;
		HeapAdd(adjHeap, &newCost, (char *) vxRight);
	    }
	}
noRight:;
    }
	...
	/* adjust cost */
	{
	    ...
	    if(DOUBLE_LESS(newCost, vxBelow->vx_cost))
	    {
		vxBelow->vx_cost = newCost;
		HeapAdd(adjHeap, &newCost, (char *) vxBelow);
	    }
	}
noBelow:;
    }
	...
	/* adjust cost */
	{
	    ...
	    if(DOUBLE_LESS(newCost, vxLeft->vx_cost))
	    {
		vxLeft->vx_cost = newCost;
		HeapAdd(adjHeap, &newCost, (char *) vxLeft);
	    }
	}
noLeft:;
    }
    ...
    Point *point;
{
	...
	hCost = DoubleMultII(e->e_hCost, ABS(point->p_x - e->e_x0));
	vCost = DoubleMultII(e->e_vCost, ABS(point->p_y - e->e_y0));

	DOUBLE_ADD(cost, hCost, vCost);
	DOUBLE_ADD(cost, cost, e->e_cost0);

	if(DOUBLE_LESS(cost, bestCost))
	    bestCost = cost;
    }
    ...
    FILE *fd;
{
	...
	fprintf(fd,"\ntile %x\t\t  (x: %d to %d, y: %d to %d)\n",
		(int) tile, r.r_xbot, r.r_xtop, r.r_ybot, r.r_ytop);
	fprintf(fd,"\thcost = %d ", 
		tilec->tc_hCost); 
	fprintf(fd,"vcost = %d \n", 
		tilec->tc_vCost);
	{
	    ...
	    {
		DoubleString(e->e_cost0,str);
		fprintf(fd,"\t\t%s + ABS(x - %d)*%d + ABS(y - %d)*%d\n",
			 str,e->e_x0,e->e_hCost,
			 e->e_y0,e->e_vCost);
	    }
	}
    }
		...
		tilec->tc_vCost);
	{
	    ...
	    {
		DoubleString(e->e_cost0,str);
		TxPrintf("\t\t%s + ABS(x - %d)*%d + ABS(y - %d)*%d\n",
			 str,e->e_x0,e->e_hCost,
			 e->e_y0,e->e_vCost);
	    }
	}
