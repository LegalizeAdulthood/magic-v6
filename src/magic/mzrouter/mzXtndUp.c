 ...
 * mzExtendUp.c --
 *
 * Code for finding next interesting point up.
 *
 *     ********************************************************************* 
 ...
 */
#ifndef lint
static char rcsid[] = "$Header: mzXtndUp.c,v 6.1 90/08/28 19:36:02 mayo Exp $";
#endif  not lint
 ...
 * mzExtendUp --
 *
 * Find next interesting point up.
 *
 * Results:
 *      None.
 *
    ...
    register RoutePath *path;
{
	...
	while(!covered)
	{
			...
			RIGHT(tpBounds)<=mzRouteUse->cu_def->cd_bbox.r_xtop)
		{
		    ...
		    p.p_y = MAX(BOTTOM(tpBounds), pOrg.p_y);
		...
		if(covered)
		{
		    NEXT_TILE_UP(tpBounds, tpBounds, pOrg.p_x);
		}
	    }
	}

	/* also get next tile over */ 
	NEXT_TILE_UP(tpNext, tpThis, pOrg.p_x);

        if(TiGetType(tpNext)!=TT_SPACE)
	{
	    /* path blocked */
	    if(BOTTOM(tpNext)==pStep.p_y)
	    {
		/* pOrg right up against block */
		if(TiGetType(tpNext)==TT_BOTTOM_WALK)
		{
		...
		PRUNE_TO_MIN(pNew.p_y, BOTTOM(tpNext)-1, reasons, RC_JOG);
	    }
	}
	else
	{
	    /* path not blocked */
	    if((RIGHT(tpNext)<RIGHT(tpThis) || 
		LEFT(tpNext)>LEFT(tpThis)) &&
	       TOP(tpThis)-1>pOrg.p_y)
	    {
		...
		PRUNE_TO_MIN(pNew.p_y, BOTTOM(tpNext), reasons, RC_JOG);
		pNew.p_y = BOTTOM(tpNext);
	    }
	}
	upEndJog:;
    }
	...
	else
	{
		...
		/* org point should not be blocked */
		ASSERT(TiGetType(tpThis)==TT_SPACE,"mzExtendUp, other");
    ...
    {
	int *yAlign = &(mzNLGetContainingInterval(&mzYAlignNL,pOrg.p_y)[1]);
	PRUNE_TO_MIN(pNew.p_y, *yAlign, reasons, RC_ALIGNGOAL);
    }
	...
	tp = TiSrPointNoHint(mzHHintPlane, &pOrg);
	PRUNE_TO_MIN(pNew.p_y, TOP(tp), reasons, RC_HINT);
    }
	...
	if(BOTTOM(tp) > pOrg.p_y)
	    /* prune to beginning of tile */
	    PRUNE_TO_MIN(pNew.p_y, BOTTOM(tp), reasons, RC_ROTBEFORE);
	else
	    /* prune to end of tile */
	    PRUNE_TO_MIN(pNew.p_y, TOP(tp)-1, reasons, RC_ROTINSIDE);
    }
	...
	for (cL=path->rp_rLayer->rl_contactL; cL!=NULL; cL=LIST_TAIL(cL))
	{
		...
		if(BOTTOM(tp) > pOrg.p_y)
		    /* prune to beginning of tile */
		    PRUNE_TO_MIN(pNew.p_y, BOTTOM(tp), reasons, RC_CONTACT);
		else
		    /* prune to end of tile */
		    PRUNE_TO_MIN(pNew.p_y, TOP(tp)-1, reasons, RC_CONTACT);
	    }
	    else
	    {
		/* BLOCK TILE - so prune to just beyond tile */
		PRUNE_TO_MIN(pNew.p_y, TOP(tp), reasons, RC_CONTACT);
	    }
	}
    }
