...
#ifndef lint
static char rcsid[] = "$Header: net2ir.c,v 6.0 90/08/28 19:08:05 mayo Exp $";
#endif lint
    ...
    char *argv[];
{
    ...
    if (argc != 3)
    {
	(void) fprintf(stderr, "Usage: net2ir feedfile netfile\n");
	exit (1);
    }
...
getfirst:
	...
	MALLOC(LabInfo *, li, sizeof (LabInfo));
	...
	if (nterms >= 2)
	{
	    (void) fprintf(stderr, "Net with >2 terms ignored: %s\n", line1);
	    continue;
	}
	...
	{
	    (void) fprintf(stderr, "No location for terminal %s\n", line1);
	    continue;
	}
