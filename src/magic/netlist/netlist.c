...
#ifndef	lint
static char rcsid[] = "$Header: netlist.c,v 6.1 90/08/28 19:25:50 mayo Exp $";
#endif	lint
...
#define	NETSIZE(r)	((r)->r_xtop - (r)->r_xbot + (r)->r_ytop - (r)->r_ybot)
    ...
    NLNetList *netList;
{
    ...
    MALLOC(NLTerm *, term, sizeof (NLTerm));
    ...
    NLTerm *term;	/* Prepend new NLTermLoc to this terminal */
{
    ...
    MALLOC(NLTermLoc *, loc, sizeof (NLTermLoc));
    loc->nloc_term = term;
    loc->nloc_rect = *area;
    ...
    Heap *netHeap;
{
	...
	{
	    int i = NETSIZE(&r);
	    HeapAdd(netHeap, &i, (ClientData) net);
	}
    }
}

