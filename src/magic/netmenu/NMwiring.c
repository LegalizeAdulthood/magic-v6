...
#ifndef lint
static char rcsid[] = "$Header: NMwiring.c,v 6.0 90/08/28 18:50:58 mayo Exp $";
#endif  not lint
...
NMRipup()
{
	...
	(void) DBEraseLabel(EditCellUse->cu_def, &list->nmwa_area, &maskBits);
    ...
    char **netPtr;		/* Pointer to pointer to net name. */
{
	...
	nmwNetTermFunc, (ClientData) netPtr) != 0)
    {
    ...
    int (*func)();		/* Processing function for each tile. */
{
	...
	0, &tpath, nmwVerifyLabelFunc2, (ClientData) func);
    return 0;
}

/*
    ...
    FILE * fp;
{
	...
	saveV=nmVCount-saveV;
        (void)fprintf(fp,"Net %s total: %d;  Metal: %d;  Poly: %d;  Vias: %d\n",
		name,
