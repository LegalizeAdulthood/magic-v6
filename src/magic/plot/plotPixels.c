...
#ifndef lint
static char rcsid[]="$Header: plotPixels.c,v 6.0 90/08/28 18:51:56 mayo Exp $";
#endif  not lint
...
#include "router.h"
#include "styles.h"
/* for grStyleTable and associated structure:  */
...
extern int swathY;		/* The y-coordinate in raster coordinates that
				 ...
				 * always >= 0.
				 */
extern int scale;		/* How many (2**scaleShift)-ths of a pixel
...
static int crossSize;		/* Length of each arm of the crosses used
				 ...
				 */
static Font *labelPixFont;	/* Font to use when rendering labels. */
static Font *cellNamePixFont;	/* Font to use when rendering cell names. */
static Font *cellIdPixFont;	/* Font to use when rendering cell ids. */

/*
 * ----------------------------------------------------------------------------
 *	PlotPixTechInit --
...
PlotPixPoint(raster, x, y, style)
    register PixRaster *raster;	/* Raster containing pixel to be
				 * filled.
				 */
					 ...
					 */
{
	...
	for (curStyle=0; curStyle<MAXTILESTYLES; curStyle++)
	{
...
register PixRaster *pr;		/* Pointer to PixRaster whose bits are
				 * to be filled in.
				 */
    register Rect *area;	/* Area to be filled in pixel coords.
    ...
    FILE *file;			/* File stream on which to dump it. */
{
    ...
    {
	(Void)GrGetColor(pr->pix_pixels[i], &r, &g, &b);
...
PlotPixRasterText(raster, clip, font, string, point, style)
    register PixRaster *raster;	/* Raster whose bits are to be filled in. */
    Rect *clip;			/* Area to which to clip the text.  Must be
				 * entirely within the area of the raster.
				 */
    Font *font;			/* Font to use for rasterizing string.  Must
				 ...
				 */
{
    ...
    {
	int cBytesPerLine, i;
	register struct dispatch *d;	/* Descriptor for current character. */
