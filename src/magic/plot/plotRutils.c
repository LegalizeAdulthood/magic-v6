...
#ifndef lint
static char rcsid[]="$Header: plotRutils.c,v 6.0 90/08/28 18:52:02 mayo Exp $";
#endif  not lint
...
Font *PlotFontList;		/* Linked list of all fonts that have
				 * been read in so far.
				 */

/*
...
PlotFillRaster(raster, area, stipple)
    register Raster *raster;		/* Pointer to raster whose bits are
					 * to be filled in.
					 */
    ...
    char *name;			/* Name of font file. */
{
    ...
    new = (Font *) mallocMagic(sizeof(Font));
    new->fo_name = NULL;
...
#ifdef BIG_ENDIAN
    new->fo_hdr.magic = (PlotSwapShort(new->fo_hdr.magic));
...
PlotTextSize(font, string, area)
    Font *font;			/* Font to use in computing text size. */
    char *string;		/* String to compute size of. */
    register Rect *area;	/* Place to store bounding box. */
{
    register int x;
...
PlotRasterText(raster, clip, font, string, point)
    register Raster *raster;	/* Raster whose bits are to be filled in. */
...
PlotRastPoint(raster, x, y)
    register Raster *raster;		/* Raster containing pixel to be
					 * filled.
					 */
...
int VersHeader[VERSATEC_BLOCK/sizeof(int)] = {VERSATEC_MAGIC_WORD};
