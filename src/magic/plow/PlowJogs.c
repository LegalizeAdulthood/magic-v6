...
#ifndef lint
static char rcsid[]="$Header: PlowJogs.c,v 6.0 90/08/28 18:52:55 mayo Exp $";
#endif  not lint
    ...
    register Outline *outline;
{
    ...
    if (TiGetType(outline->o_outside) != TT_SPACE)
	return (1);
...
{
    /* Stop if we're no longer adjacent to space */
    if (TiGetType(outline->o_inside) != TT_SPACE)
	return (1);
...
plowJogMoveFunc(edge)
    register Edge *edge;
{
    register Edge *origEdge = jogEdge;
