...
RuleTableEntry *plowCellRulesPtr = plowCellRulesTbl;
    ...
    /* Imported rules */
extern Void prClearUmbra();
extern Void prUmbra();
extern Void prPenumbraTop(), prPenumbraBot();
extern Void prFixedPenumbraTop(), prFixedPenumbraBot();
extern Void prSliverTop(), prSliverBot();
extern Void prInSliver();
extern Void prIllegalTop(), prIllegalBot();
extern Void prCoverTop(), prCoverBot();
extern Void prFixedLHS(), prFixedRHS(), prFixedDragStubs();
extern Void prContactLHS(), prContactRHS();
extern Void prFindCells();
extern Void prCell();
    ...
    int direction;	/* Pull all jogs in this direction to straighten them */
{
    ...
    plowYankedArea.r_xbot = yankArea.r_xbot - TechHalo;
    plowYankedArea.r_ybot = yankArea.r_ybot - TechHalo;
    plowYankedArea.r_xtop = yankArea.r_xtop + TechHalo;
    plowYankedArea.r_ytop = yankArea.r_ytop + TechHalo;
    ...
    int distance;		/* Plow distance */
{
    ...
    save = realUse->cu_delta;
    realUse->cu_delta = distance;
    (void) DBCellEnum(plowYankDef, plowFindSelCell, (ClientData) realUse);
    realUse->cu_delta = save;
    ...
    CellUse *editUse;	/* Cell from the original cell def */
{
    ...
    edge.e_newx = yankUse->cu_bbox.r_xtop + editUse->cu_delta;
...
plowPastBoundary(def, edge, pmove)
    ...
    register int *pmove;	/* Updated to be the maximum distance by
				 * which something moves in an illegal area.
				 */
{
    register PlowBoundary *pb;
    int delta;
    bool ret;
    Rect r;
...
plowInitialCell(cellTile, plowRect)
    Tile *cellTile;
    register Rect *plowRect;
{
    register CellTileBody *ctb;
    ...
    Rect *changedArea;	/* Include any additional area changed in this area */
{
    ...
    if (edge->e_use)
    {
	if (amountToMove > edge->e_use->cu_delta)
	{
	     ...
	     */
	    edge->e_use->cu_delta = amountToMove;
	    r = edge->e_use->cu_bbox;
...
plowMoveEdge(edge)
    register Edge *edge;	/* Edge to be moved */
{
    register Plane *plane = plowYankDef->cd_planes[edge->e_pNum];
    register Tile *tp, *tpL;
    Point p;
...
plowMergeTop(tp, plane)
    ...
    Plane *plane;
{
    register Tile *tpRT = RT(tp);

    if (TiGetType(tp) == TiGetType(tpRT)
...
plowMergeBottom(tp, plane)
    ...
    Plane *plane;
{
    register Tile *tpLB = LB(tp);

    if (TiGetType(tp) == TiGetType(tpLB)
 ...
 * outside of it.  Currently, TechHalo is sufficient; only the rules
...
plowInitRule(rtePtr, rteEnd, whichRules, proc, name, ltypes, rtypes)
    ...
    Void (*proc)();		/* Procedure implementing the rule */
