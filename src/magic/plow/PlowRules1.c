...
#ifndef lint
static char rcsid[]="$Header: PlowRules1.c,v 6.0 90/08/28 18:53:10 mayo Exp $";
#endif  not lint
...
plowPenumbraTopProc(outline, ar)
    register Outline *outline;		/* Segment along penumbra border */
    register struct applyRule *ar;	/* Info needed for shadow search */
{
    register Edge *movingEdge = ar->ar_moving;
    register PlowRule *pr = ar->ar_rule;
    Rect searchArea;
    int ret = 0;
...
plowPenumbraRule(impactedEdge, ar)
    register Edge *impactedEdge;	/* Edge found by shadow search */
    register struct applyRule *ar;	/* Edge causing the shadow search, and
...
plowSliverTopMove(outline, ar)
    register Outline *outline;		/* Segment of outline being followed */
    ...
    register struct applyRule *ar;
{
	...
	case GEO_NORTH:
		...
		ar->ar_slivtype = TiGetType(outline->o_outside);
    ...
    if (plowSliverApplyRules(ar, TiGetType(outline->o_inside),
    ...
    register struct applyRule *ar;
{
	...
	case GEO_SOUTH:
		...
		ar->ar_slivtype = TiGetType(outline->o_inside);
    ...
    /* Apply the plowing rules to see if this is a sliver */
    if (plowSliverApplyRules(ar, TiGetType(outline->o_outside),
