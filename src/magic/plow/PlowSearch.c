...
#ifndef lint
static char rcsid[]="$Header: PlowSearch.c,v 6.0 90/08/28 18:53:19 mayo Exp $";
#endif  not lint
...
#define	IsInside(tp, out) TTMaskHasType(&(out)->o_insideTypes, TiGetType(tp))
...
register Tile *in = (o)->o_nextIn; \
register Tile *out = (o)->o_nextOut; \
switch ((o)->o_nextDir) \
{ \
...
plowShadowRHS(tp, s, bottomLeft)
    register Tile *tp;		/* Tile whose RHS is to be followed */
    register struct shadow *s;	/* Shadow search argument */
    int bottomLeft;		/* Bottom of 'tp', clipped to area */
{
	...
	if (bottom < s->s_edge.e_ytop)
	{
	    ...
	    if (!TTMaskHasType(&s->s_okTypes, TiGetType(tpR)))
	    {
		/* Left, right hand types */
		s->s_edge.e_ltype = TiGetType(tp);
		s->s_edge.e_rtype = TiGetType(tpR);
    ...
    int bottomLeft;		/* Bottom of 'tp', clipped to area */
{
	...
	{
	    /* If tpR is not in okTypes, pass the edge to the client function */
	    if (TiGetType(tp) != TiGetType(tpR)
		&& (!TTMaskHasType(&s->s_okTypes, TiGetType(tpR))
			|| !TTMaskHasType(&s->s_okTypes, TiGetType(tp))))
	    {
		/* Left, right hand types */
		s->s_edge.e_ltype = TiGetType(tp);
		s->s_edge.e_rtype = TiGetType(tpR);
...
plowShadowLHS(tp, s, topRight)
    register Tile *tp;		/* Tile whose LHS is to be followed */
    register struct shadow *s;	/* Shadow search argument */
    int topRight;		/* Top of 'tp', clipped to area */
{
	...
	if (top > s->s_edge.e_ybot)
	{
	    ...
	    if (!TTMaskHasType(&s->s_okTypes, TiGetType(tpL)))
	    {
		/* Left, right hand types */
		s->s_edge.e_ltype = TiGetType(tpL);
		s->s_edge.e_rtype = TiGetType(tp);
...
plowAtomize(pNum, rect, proc, cdata)
    int pNum;			/* Plane from plowYankDef to search */
    register Rect *rect;	/* LHS is the geometrical edge we search; each
