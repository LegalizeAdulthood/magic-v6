...
#ifndef lint
static char rcsid[]="$Header: PlowTest.c,v 6.0 90/08/28 18:53:27 mayo Exp $";
#endif  not lint
    ...
    TxCommand *cmd;
{
	...
	case PC_PRINT:
		...
		TRAILING(tp), LEADING(tp), DBTypeLongName(TiGetType(tp)));
	    break;
	case PC_WHENTOP:
	    if (cmd->tx_argc == 2)
	    {
    ...
    Rect *area;
{
    ...
    scx.scx_area.r_xbot = area->r_xbot - TechHalo;
    scx.scx_area.r_ybot = area->r_ybot - TechHalo;
    scx.scx_area.r_xtop = area->r_xtop + TechHalo;
    scx.scx_area.r_ytop = area->r_ytop + TechHalo;
    ...
    Rect *clipArea;
{
    ...
    (void) sprintf(mesg, "%s/%s/%s segment in=%s out=%s",
		...
		DBTypeLongName(TiGetType(outline->o_inside)),
		DBTypeLongName(TiGetType(outline->o_outside)));
