...
#ifndef lint
static char rcsid[]="$Header: PlowYank.c,v 6.0 90/08/28 18:53:33 mayo Exp $";
#endif  not lint
				 ...
				 */
{
     ...
     *		Use the original set to update cu_deltas and trailing tile
     *			coordinates.
     *
...
plowYankUpdateCell(yankChildUse)
    register CellUse *yankChildUse;	/* Use in the yank cell */
{
    register CellUse *spareChildUse;
    int savedelta;

    savedelta = yankChildUse->cu_delta;
		...
		&& strcmp(spareChildUse->cu_id, yankChildUse->cu_id) == 0)
	{
	    ...
	    yankChildUse->cu_delta = savedelta;
...
plowYankUpdatePaint(yankTp, pNum)
    register Tile *yankTp;
    int pNum;
{
    register Tile *spareTp;
    Point startPoint;
    Plane *plane;
	 ...
	 */
	if (TiGetType(spareTp) == TiGetType(yankTp))
	{
    ...
    Rect *origArea;	/* Area in original def that was modified */
{
	...
	if (lu.lu_adjust)
	{
	    ...
	    DBUndoEraseLabel(origDef, &origLab->lab_rect, origLab->lab_pos,
				origLab->lab_text, origLab->lab_type);
	    GeoTransRect(&plowInverseTrans, &lu.lu_rect, &origLab->lab_rect);
	    DBUndoPutLabel(origDef, &origLab->lab_rect, origLab->lab_pos,
				origLab->lab_text, origLab->lab_type);
 ...
 * Update the positions of each cell moved by plowing.  The cu_delta
 * fields of various cells in the cell plane of yankDef have been updated
 * as a result of plowing.  For each cell with a non-zero cu_delta, we
...
plowUpdateCell(use, origDef)
    CellUse *use;	/* Use from yank def; corresponding use in 'def' will
			 * be moved if use->cu_delta is non-zero.
			 */
