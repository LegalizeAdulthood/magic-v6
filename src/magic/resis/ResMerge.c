...
#ifndef lint
static char rcsid[] = "$Header: ResMerge.c,v 6.0 90/08/28 18:54:37 mayo Exp $";
#endif  not lint
...

{
     ...
     {
     	  resResistor	*rr1 = rcell1->re_thisEl;
     ...
     if ((resptr->rn_te == NULL) && (resptr->rn_why != RES_NODE_ORIGIN) && (status == UNTOUCHED))
     {
		...
		if (rr1->rr_connection1 == resptr)
		{
		     ...
		     if (rr2->rr_connection1 == resptr)
		     {
			      ...
			      rr2->rr_connection2)
			  {
			       ...
			       resptr2->rn_float.rn_area += rr1->rr_float.rr_area+rr2->rr_float.rr_area+resptr->rn_float.rn_area;
			       ResEliminateResistor(rr1,&ResResList);
...
{
     register resResistor	*rr1,*rr2,*rr3;
     int		status=UNTOUCHED;
     		...
     		rcell1->re_nextEl != NULL; rcell1 = rcell1->re_nextEl)
     {
	  	...
	  	rcell2 != NULL; rcell2 = rcell2->re_nextEl)
	  {
	       ...
	       if(TTMaskHasType(ResNoMergeMask+rr1->rr_tt,rr2->rr_tt))continue;
	       		...
	       		rcell3 != NULL; rcell3 = rcell3->re_nextEl)
	       {
		    ...
		    MALLOC(resNode  *, n3, sizeof(resNode));
 ...
 */
	...
	
 {
      ...
      if (node2->rn_client != NULL)
      {
	    FREE((char *)node2->rn_client);
	    node2->rn_client = NULL;
      }
      {
	    node2->rn_re = (resElement *) MINFINITY;
	    node2->rn_ce = (cElement   *) MINFINITY;
	    node2->rn_je = (jElement   *) MINFINITY;
	    node2->rn_te = (tElement   *) MINFINITY;
	    node2->rn_more = (resNode   *) MINFINITY;
	    node2->rn_less = (resNode   *) MINFINITY;
      }
      FREE((char *)node2);
 }
        ...
        resNode **homelist2;
{
     ...
     while (resptr->rn_je != NULL)
     {
	     ...
	     FREE((char *)jcell->je_thisj);
	     FREE((char *)jcell);
     }
     if (junk == TRUE)
     {
	  if (resptr->rn_client != NULL)
	  {
	       FREE((char *)resptr->rn_client);
	       resptr->rn_client = NULL;
	  }
	  ...
	  {
	       resptr->rn_re = (resElement *) MINFINITY;
	       resptr->rn_ce = (cElement   *) MINFINITY;
	       resptr->rn_je = (jElement   *) MINFINITY;
	       resptr->rn_te = (tElement   *) MINFINITY;
	       resptr->rn_more = (resNode   *) MINFINITY;
	       resptr->rn_less = (resNode   *) MINFINITY;
	  }
	  FREE((char *)resptr);
     }
}
