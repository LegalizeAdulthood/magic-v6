...
#ifndef lint
static char rcsid[] = "$Header: ResReadSim.c,v 6.0 90/08/28 18:54:41 mayo Exp $";
#endif  not lint
 ...
 
{
     ...
     char *cp;
         ...
         while (gettokens(line,fp) != NULL)
         {
	      ...
	      node->location.p_x = 
	      	atoi(line[NODENODEX])/lambda;
	      node->location.p_y = 
	      	atoi(line[NODENODEY])/lambda;
#ifdef ARIEL	      
...

{
     ...
     MALLOC(RTran *, transistor, sizeof(RTran));
     ...
     else
     {
	   ...
	   transistor->resistance = atof(line[RTRAN_LENGTH])* rpersquare/atof(line[RTRAN_WIDTH]);
     }
...

{
     ...
     ResSimNode		*node1,*node2,*ResInitializeNode();
     
     if (line[COUPLETERMINAL1][0] == NULL || line[COUPLETERMINAL2][0] == NULL)
     {
     ...
     {
          node1->capacitance += atof(line[COUPLEVALUE]);
          ...
          node2->capacitance += atof(line[COUPLEVALUE]);
          return(0);
     }
     if (strcmp(line[COUPLETERMINAL2],"GND") == 0 )
     {
     ...
     {
          node1->cap_vdd += atof(line[COUPLEVALUE]);
	  return(0);
     }
     entry2 = HashFind(&ResNodeTable,line[COUPLETERMINAL2]);
     node2 = ResInitializeNode(entry2);
     ...
     {
          node2->cap_vdd += atof(line[COUPLEVALUE]);
	  return(0);
     }
     node1->cap_couple += atof(line[COUPLEVALUE]);
     node2->cap_couple += atof(line[COUPLEVALUE]);
     return(0);
...

{
     ...
     node->resistance = atof(line[NODERESISTANCE]);
     return(0);
}
	...
	char	*filename;
{
     ...
     while (gettokens(line,fp) != NULL)
     {
	  ...
	  MALLOC(ResFixPoint *, thisfix, sizeof(ResFixPoint)+strlen(label));
	...
	HashEntry	*entry;
{
	ResSimNode	*node;
     ...
     {
	  MALLOC(ResSimNode *, node, sizeof(ResSimNode));
