...
#ifndef lint
static char rcsid[] = "$Header: ResSimple.c,v 6.0 90/08/28 18:54:46 mayo Exp $";
#endif  not lint
#include <stdio.h>
#include <ctype.h>
#include <math.h>
     ...
     */
	  ...
	  */
	  ...
	  else
	  {
          ...
          MALLOC(resElement *,resisptr,sizeof(resElement));
...

{
     ...
     MALLOC(RCDelayStuff *, myC, sizeof(RCDelayStuff));
...

{
	 ...
	 node->rn_re->re_thisEl->rr_connection2 == node)
     {
	  ...
	  else if (((RCDelayStuff *)(node->rn_client))->rc_Tdi < minTdi)
	  {
	       ...
	       ResMergeNodes(currentRes->rr_connection1,currentRes->rr_connection2,nodelist1,nodelist2);
...

{
     ...
     for (re = node->rn_re;re;re=re->re_nextEl)
     {
	  ...
	  {
	       pKey.hu_int = node->rn_noderes+res->rr_value;
	       HeapAdd(&ResistorHeap,&pKey,(char *)res);
	  }
     }
}
resPathRes(res)
