...
#ifndef lint
static char rcsid[]="$Header: rtrDcmpose.c,v 6.0 90/08/28 18:55:44 mayo Exp $";
#endif  not lint
 ...
 *
 * Results:
 *	Return 0 if the corner is not convex or a legal boundary already
 *	extends from the corner.  Otherwise return 1.
 *
 * Side effects:
    ...
    Tile *tiles[];	/* Return pointers to found space tiles */
{
    ...
    if( (TiGetBody(tile) != NULL) || (LEFT(tile) == point->p_x) ||
    ...
    int	corner;		/* Corner of tile to process */
{
	...
	while(TRUE)
	{
	    ASSERT(TiGetBody(tile) ==  NULL,
		    "rtrMerge:  merge cell tile");
	    new = TiSplitX(tile, curPt.p_x);
	    ASSERT(TiGetBody(new) ==  NULL, "rtrMerge:  merge cell new");
...
{
    register Tile *current = tiles[1], *next;
    int x, yStart, flag;
    Point p;
...
rtrXDist(tiles, x, isRight)
    register Tile *tiles[];	/* Space tiles bordering the corner */
    register int x;		/* Starting x for distance calculation */
...
rtrMerge(tup, tdn, plane)
    register Tile *tup, *tdn;
    register Plane *plane;
{
    register Tile *side;

    /* Skip if either is a cell tile */
    if (TiGetBody(tup) != NULL || TiGetBody(tdn) != NULL)
	return;
