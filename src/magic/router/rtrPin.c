...
#ifndef lint
static char rcsid[] = "$Header: rtrPin.c,v 6.0 90/08/28 18:55:55 mayo Exp $";
#endif  not lint
...
RtrPointToPin(ch, side, point)
    register GCRChannel *ch;	/* The channel containing the point */
    int side;			/* Side of ch that point lies on */
    register Point *point;	/* The point to be converted to a pin */
{
    register int coord;
    ...
    int nPins;		/* Number of internal pins (not counting pins[0]) */
{
    ...
    register GCRPin *pin, *opin, *linked;
    GCRPin *lastPin;
...
rtrPinArrayLink(pins, nPins)
    ...
    int nPins;
{
    register GCRPin *pin, *lastPin, *lastValid;
