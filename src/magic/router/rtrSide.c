...
#ifndef lint
static char rcsid[]="$Header: rtrSide.c,v 6.1 90/08/28 19:28:26 mayo Exp $";
#endif  not lint
 ...
 * rtrEnumSides --
 *
 * Enumerate each collinear boundary between a cell and empty space
 * in the cell 'def' that lies entirely within the area 'area'.
 * Collinear boundaries occur when, for example, two vertically
 ...
 *	done (MINFINITY).  However, the cell plane of use->cu_def
			 ...
			 */
{
	...
	(void) TiSrArea((Tile *) NULL, rtrSideTransPlane, area,
	    rtrSideInitClient, (ClientData) MINFINITY);
	SigEnableInterrupts();
    }
    ...
    register Tile *tile;
{
    ...
    if (yprev <= ybot || tile->ti_body == NULL
    ...
    ytop = MIN(ytop, yprev);
    while (tp->ti_body == NULL && TOP(tp) > rtrSideArea.r_ybot
			       && LEFT(tp) == RIGHT(tile)
			       && RIGHT(tp) >= x)
    {
	tpB = tp;
