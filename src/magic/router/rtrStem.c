...
#ifndef lint
static char rcsid[]="$Header: rtrStem.c,v 6.0 90/08/28 18:56:01 mayo Exp $";
#endif  not lint
...
GCRChannel *rtrStemSearch();
GCRPin *rtrStemTip();
GCRPin *rtrStemTryPin();

/*
 * ----------------------------------------------------------------------------
 *
    ...
    NLNet *net;		/* For marking pin */
{
    ...
    if (termWidth == 0)
    {
	(void) sprintf(errorMesg,
	    "Can't use point label for terminal:  use line or box instead");
	goto fail;
    }
    ...
    {
	sprintf(errorMesg, "%s %s layer: must connect to %s or %s",
	    "Can't have terminal on", DBTypeLongName(type),
	    DBTypeLongName(RtrMetalType), DBTypeLongName(RtrPolyType));
	goto fail;
    }
...
rtrStemTip(loc, si, use)
    register NLTermLoc *loc;	/* Location whose stem tip is being found */
    ...
    CellUse *use;
{
	...
	MALLOC(NLTermLoc *, nloc, sizeof (NLTermLoc));
	*nloc = *loc;
	loc->nloc_next = nloc;
	loc = nloc;
    }
...
{
    register Rect *area = &loc->nloc_rect;
...
RtrStemPaintExt(use, loc)
    CellUse *use;
    register NLTermLoc *loc;	/* Terminal whose stem is to be painted */
{
