...
#ifndef lint
static char rcsid[]="$Header: rtrTravers.c,v 6.1 90/08/28 19:28:29 mayo Exp $";
#endif  not lint
    ...
    struct rtrTileStack *ts;
{
	...
	if ( DBConnPlanes[TiGetType(tp[0])] &&
	     (TiGetType(tp[1]) == rtrReplace) &&
	     DBConnPlanes[TiGetType(tp[2])] )
	{
	    int plane;
	    Rect area;
