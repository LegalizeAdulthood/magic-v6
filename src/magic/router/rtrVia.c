...
#ifndef lint
static char rcsid[]="$Header: rtrVia.c,v 6.1 90/08/28 19:28:37 mayo Exp $";
#endif  not lint
    ...
    struct srinfo *si;
{
    ...
    if ( tile != si->si_tile )
    {
	MALLOC( struct paintlist *, pl, sizeof(*pl));
...
rtrListArea(tile, oldType, newType, deltax, deltay)
    ...
    int deltay;
{
    register struct arealist *ap;

    MALLOC( struct arealist *, ap, sizeof(*ap));
...
rtrListVia(tile)
    Tile *tile;
{
    register struct vialist *vp;

    MALLOC( struct vialist *, vp, sizeof(*vp));
