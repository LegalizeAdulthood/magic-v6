...
CellDef *newdef;
{
	...
	MALLOC(DefListElt *, DefList, sizeof(DefListElt));
	DefList->dl_def = newdef;
	DefList->dl_next = (DefListElt *) NULL;
	return;
    }
    else {
	MALLOC(DefListElt *, d, sizeof(DefListElt));
	d->dl_next = DefList;
	d->dl_def = newdef;
	DefList= d;
    }
...
Void SimInitConnTables()
{
    ...
    {
	TTMaskSetMask( &SimSDMask, &ExtCurStyle->exts_transSDTypes[t] );
	...
	if( TTMaskHasType( &SimTransMask, t ) )
	{
	    ...
	    {
		if( TTMaskHasType( &ExtCurStyle->exts_transSDTypes[t], sd ) )
		{
    ...
    SimTrans	*trans;
{
    ...
    int		pNum = DBPlane( TiGetType( tile ) );
    int		i;
...
int SimTransistorTile( tile, pNum, arg )
    Tile	*tile;
    int		pNum;
    FindRegion	*arg;
{
	...
	(void) extEnumTilePerim( tile,
	  ExtCurStyle->exts_transSDTypes[TiGetType( tile )],
...
NodeSpec *SimFindOneNode( sx, tile )
    SearchContext	*sx;
    Tile		*tile;
{
	...
	/* Allocate a new node */
    MALLOC(NodeRegion *, reg, sizeof(NodeRegion) );
    ...
    arg.fra_def = def;
    arg.fra_pNum = DBPlane( TiGetType( tile ) );
	...
	arg.fra_pNum = DBPlane( TiGetType( gateTile ) );
	arg.fra_uninit = (ClientData) extUnInit;
	arg.fra_region = (Region *) reg;
	...
	arg.fra_connectsTo = &SimTransMask;
	arg.fra_pNum = DBPlane( TiGetType( sdTile ) );
 ...
 *	the ndoe region data structure which contains the labels for a
...
char *SimGetNodeName(sx, tp, path)
