 ...
 *	rsim/irsim.  If this variable does not exist, then ~cad/bin/irsim
 *	is then used.
 *
 * Results:
...

{
     ...
     * try to open the one located in ~cad/bin.
     */
	...
	src = cad;
	dst = rsimfile;
	if (PaConvertTilde(&src, &dst, 100) == -1) {
	    TxError ("Could not find ~cad\n");
	    return(FALSE);
	}
	strcat(rsimfile, "/bin/irsim");
    }
...

{
	...
	if (TxGetLine(keyBoardBuf, BUF_SIZE) == NULL) keyBoardBuf[0] = 0;
...
static bool InitRsim( hello_msg )
    char  *hello_msg;
{
    char	buff[LINEBUF_SIZE];
    char	*last;
    int		nchars;
...
{
static char msg[] = 
"The simulator must be running before this command can be executed.  To do\n\
this enter the command \"rsim <options> <filename>\".  To escape back to\n\
 ...
 *	places the charcters into a buffer pointed by pLastChar.
...

{
  ...
  try_again:
    ...
    nr = nex = 1 << pipeIn;
    nfd = pipeIn + 1;
    n = select( nfd, &nr, (int *) NULL, &nex, (int *) NULL );
    if( n < 0 )
    {
	if( errno == EINTR )
	{
 ...
 *	otherwise it is FALSE.  If we find a line containing only
 * 	the rsim prompt, replyLine points to a NULL pointer and
 *	true is returned.
 *
 * Side effects:
