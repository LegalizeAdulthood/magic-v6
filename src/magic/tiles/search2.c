...
#ifndef	lint
static char rcsid[] = "$Header: search2.c,v 6.0 90/08/28 18:02:48 mayo Exp $";
#endif	not lint
...
{
    Point here;
    register Tile *tp, *enumTR, *enumTile;
...
tiSrAreaEnum(enumRT, enumBottom, rect, func, arg)
    register Tile *enumRT;	/* TR corner stitch of tile just enumerated */
    int enumBottom;		/* Bottom coordinate of tile just enumerated */
    register Rect *rect;	/* Area to search */
    int (*func)();		/* Function to apply at each tile */
    ClientData arg;		/* Additional argument to pass to (*func)() */
{
    register Tile *tp, *tpLB, *tpTR;
