...
register Transform *t;		/* A description of the mapping from the
				 * coordinate system of p1 to that of p2.
				 */
register Point *p1, *p2;	/* Pointers to two points; p1 is the old
...
register Transform *t;		/* A description of the mapping from the
				 ...
				 */
register Rect *r1, *r2;		/* Pointers to two rectangles, r1 is the old
...
register Transform *trans1;	/* Transform to be translated */
int x, y;			/* Amount by which to translated */
register Transform *trans2;	/* Result transform */
{
...
register Transform *trans1;	/* Transform to be applied to translation */
register Transform *trans2;	/* Result transform */
{
    trans2->t_a = trans1->t_a;
    trans2->t_b = trans1->t_b;
...
register Transform *first;		/* Pointers to three transforms */
    ...
    int pos;
{
    switch(pos)
    {
        ...
        case GEO_SOUTHEAST: return("SOTHEAST");
...
register Transform *t;			/* Pointer to a transform */
...
void
GeoClip(r, area)
register Rect *r;		/* Rectangle to be clipped. */
register Rect *area;		/* Area against which to be clipped. */
...
void
GeoClipPoint(p, area)
register Point *p;		/* Point to be clipped. */
register Rect *area;		/* Area against which to be clipped. */
