...
#ifndef lint
static char rcsid[]="$Header: windDisp.c,v 6.0 90/08/28 19:02:18 mayo Exp $";
#endif  not lint
...
windReClip()
{
    ...
    for (w1 = windBottomWindow; w1 != (Window *) NULL; w1 = w1->w_prevWindow)
    {
	LinkedRect *tmp;
	...
	if (WindPackageType == WIND_MAGIC_WINDOWS) {
	    for (w2 = w1->w_prevWindow; w2 != (Window *) NULL; 
		    w2 = w2->w_prevWindow)
	    {
    ...
    Window *w;
{
    ...
    if (w->w_redrawAreas != NULL) return;
...
WindAreaChanged(w, area)
    Window *w;		/* The window that changed. */
...
windBarLocations(w, leftBar, botBar, up, down, right, left, zoom)
    Window *w;		/* The window under consideration. */
			 ...
			 */
{
	...
	BOT_BORDER(w);
    leftBar->r_xtop = leftBar->r_xbot + WindScrollBarWidth - 1;
	...
	LEFT_BORDER(w);
    botBar->r_ytop = botBar->r_ybot + WindScrollBarWidth - 1;
    ...
    Rect *clip;
{
    ...
    capr = w->w_allArea;
    capr.r_ybot = capr.r_ytop - TOP_BORDER(w) + 1;
	...
	if (w->w_caption != NULL) {
		...
		GEO_CENTER, GR_TEXT_DEFAULT, FALSE, &capr, (Rect *) NULL);
	};
    }

    /* right border */
    r = w->w_allArea;
    r.r_xbot = w->w_allArea.r_xtop - RIGHT_BORDER(w) + 1;
	...
	/* left border */
	r = w->w_allArea;
	r.r_xtop = w->w_allArea.r_xbot + LEFT_BORDER(w) - 1;
	r.r_ytop = w->w_allArea.r_ytop - TOP_BORDER(w);
	if (GEO_TOUCH(&r, clip)) GrClipBox(&r, STYLE_BORDER);

	/* bottom border */
	r = w->w_allArea;
	r.r_ytop = w->w_allArea.r_ybot + BOT_BORDER(w) - 1;
	if (GEO_TOUCH(&r, clip)) GrClipBox(&r, STYLE_BORDER);
    }
    else {
	...
	leftElev.r_xtop = leftBar.r_xtop - 2;
	...
	botElev.r_ytop = botBar.r_ytop - 2;
...
windUpdateFunc(tile, w)
    ...
    Window *w;			/* Window we're currently interested in. */
{
    Rect area;
    ...
    ClientData notUsed;
{
    ...
    GrLock((Window *)GR_LOCK_SCREEN, FALSE);
    GrClipBox(&area, STYLE_BACKGROUND);
    GrUnlock((Window *)GR_LOCK_SCREEN);
    return 0;
}
