...
#ifndef lint
static char rcsid[]="$Header: windMove.c,v 6.0 90/08/28 19:02:23 mayo Exp $";
#endif  not lint
...
static Window *sharedW;

/*
 * Should new windows have scroll bars?
 */
bool windDefaultScrollBars = TRUE;
...
windUnlink(w)
    Window *w;
{
    ASSERT(w != (Window *) NULL, "windUnlink");
    ASSERT(windTopWindow != (Window *) NULL, "windUnlink");
    ASSERT(windBottomWindow != (Window *) NULL, "windUnlink");
    ASSERT(windTopWindow->w_prevWindow == (Window *) NULL, "windUnlink");
    ASSERT(windBottomWindow->w_nextWindow == (Window *) NULL, "windUnlink");
	...
	if (windTopWindow == w)
	{
	    ...
	    if (windTopWindow != (Window *) NULL)
		windTopWindow->w_prevWindow = (Window *) NULL;
	}
	if (windBottomWindow == w)
	{
	    windBottomWindow = w->w_prevWindow;
	    if (windBottomWindow != (Window *) NULL)
		windBottomWindow->w_nextWindow = (Window *) NULL;
	}
    }
    else
    {
    ...
    w->w_nextWindow = (Window *) NULL;
    w->w_prevWindow = (Window *) NULL;
}

/*
 * ----------------------------------------------------------------------------
 ...
 *
 *	Make sure that the window system knows that the window is located
 *	at w->w_frameArea.  Called when w->w_frame area is changed.
 *	This is a no-op if Magic is managing it's own windows, since it
 *	runs directly off of w->w_frameArea.
 *
 * Results:
 *	None.
 *
    ...
    Window *w;
{
    ...
    ASSERT(w != (Window *) NULL, "WindDelete");
    ...
    char *argv[];
{
    ...
    w = (Window *) mallocMagic( sizeof(Window) );
    w->w_client = client;
    w->w_flags = WIND_SCROLLABLE;
    if (windDefaultScrollBars) w->w_flags |= WIND_SCROLLBARS;
...
#ifdef	SUNVIEW
    /* Don't set window positions ourselves. */
    if ((WindPackageType == WIND_SUN_WINDOWS) && isHint)
	GrSunFigureLocation(&w->w_frameArea);
#endif	SUNVIEW
    WindSetWindowAreas(w);
    ...
    if (GrCreateWindowPtr != NULL) OK &= (*GrCreateWindowPtr)(w);
...
WindOutToIn(w, out, in)
    Window *w;			/* Window under consideration */
...
void
WindUnder(w)
    Window *w;		/* the window to be moved */
{
    Rect area;
    Window *w2;
    ...
    Window *w;		/* the window to be moved */
{
	...
	default:
		...
		if ((area.r_xbot <= area.r_xtop) && (area.r_ybot <= area.r_ytop))
		    WindAreaChanged((Window *) NULL, &area);
	    }
...
WindReframe(w, r, inside, move)
    Window *w;		/* the window to be reframed */
			 ...
			 */
{
	...
	w->w_origin.p_x += xmove << 12;
	ymove = newFrameArea.r_ybot - w->w_frameArea.r_ybot;
	w->w_origin.p_y += ymove << 12;
...
windReframeFunc(area, w)
    ...
    Window *w;			/* Window in which to redisplay. */
...
void
WindFullScreen(w)
    Window *w;			/* Window to be blown up or shrunk back. */
{
    int i;
    Window *w2;
    Rect newFrameArea;
	...
	{
	    ASSERT(w2 != (Window *) NULL, "WindFullScreen");
	    w->w_oldDepth += 1;
	}
    }
