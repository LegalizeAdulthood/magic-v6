...
#ifndef lint
static char rcsid[]="$Header: wireOps.c,v 6.0 90/08/28 19:02:52 mayo Exp $";
#endif  not lint
				 ...
				 */
{
    ...
    TTMaskSetOnlyType(&mask, WireType);
    DBPaint(EditCellUse->cu_def, &editArea, WireType);
				 ...
				 */
{
    ...
    DBPaint(EditCellUse->cu_def, &tmp, contact->con_type);
    if (contact->con_surround1 != 0)
    {
	GEO_EXPAND(&tmp, contact->con_surround1, &tmp2);
	(void) GeoInclude(&tmp2, &editArea);
	DBPaint(EditCellUse->cu_def, &tmp2, contact->con_layer1);
	TTMaskSetType(&mask, contact->con_layer1);
    }
    if (contact->con_surround2 != 0)
    {
	GEO_EXPAND(&tmp, contact->con_surround2, &tmp2);
	(void) GeoInclude(&tmp2, &editArea);
	DBPaint(EditCellUse->cu_def, &tmp2, contact->con_layer2);
	TTMaskSetType(&mask, contact->con_layer2);
    }
    DBAdjustLabels(EditCellUse->cu_def, &editArea);
    DBWAreaChanged(EditCellUse->cu_def, &editArea, DBW_ALLWINDOWS, &mask);
    DRCCheckThis(EditCellUse->cu_def, TT_CHECKPAINT, &editArea);
...
WireButtonProc(w, cmd)
    Window *w;			/* Window in which button was pushed. */
